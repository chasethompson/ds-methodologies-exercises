...
"""
Utility built for logistic regression lessons, made primarily for binary classifiers,
may or may not work well with others - ie. multi-class.
"""
...
def evaluate_thresholds(y, probs):
    return pd.DataFrame([evaluate_thresold(t, y, probs) for t in np.arange(0, 1.01, 0.01)])
...
def plot_true_by_probs(y, probs, subplots=False):
    if subplots:
        ...
        axs[1].set(title="y = 1", xlabel="P(y = 1)", xlim=(0, 1), xticks=np.arange(0, 1.1, 0.1)
    else:
        probs.groupby(y).plot.hist(bin=25, alpha=0.6)
        plt.xlabel("P(y = 1)")
        plt.legend(title=y.name)


def plot_metrics_by_thresholds(y, probs, subplots=False):
    evaluation = evaluation_thresholds(y, probs)
    axs = (
...
def get_macbook_data():
    ...
    df["coolness"] = np.where(df.macbook == 1, np.random.normal(80, 10, 40), np.random.normal(40, 10, 40))
    return df